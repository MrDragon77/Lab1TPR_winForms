            ...
            while (DS.Tables["s" + cur_strat.ToString()] != null)
            {
                ...
                for (int j = 0; j < conditions; j++)
                {
                    ...
                    if (n > 0)
                    {
                        for (int i = 0; i < conditions; i++)
                        {
                            sum += Convert.ToDouble(results.Rows[i][n - 1]) * Convert.ToDouble(d.Rows[j][i]); //добавляем к ожидаемой прибыли максимальные прибыли с прошлых итераций и умножаем их на вероятности текущей стратегии (я час сидел и так и не понял, почему именно так, спасите
                        }
                    }
